...
class CustomCategoryRepo {
  ...
  Future<int> createCategory(model.CustomCategory category) async {
    final categoryId = await _db.into(_db.customCategories).insert(
  ...
  /// 앱 매핑 추가
  Future<void> _addAppMappings(int categoryId, List<String> packageNames) async {
    for (final packageName in packageNames) {
      await _db.into(_db.categoryAppMappings).insert(
  ...
  Future<bool> deleteCategory(int categoryId) async {
    final deleted = await (_db.delete(_db.customCategories)
  ...
  Future<List<model.CustomCategory>> getActiveCategories() async {
    ...
    for (final category in categories) {
      final mappings = await (_db.select(_db.categoryAppMappings)
            ..where((t) => t.categoryId.equals(category.id)))
        .get();
  ...
  Future<model.CustomCategory?> getCategory(int categoryId) async {
    final category = await (_db.select(_db.customCategories)
          ..where((t) => t.id.equals(categoryId)))
        .getSingleOrNull();
  ...
  /// 패키지명으로 카테고리 찾기
  Future<model.CustomCategory?> findCategoryByPackage(String packageName) async {
    final mapping = await (_db.select(_db.categoryAppMappings)
